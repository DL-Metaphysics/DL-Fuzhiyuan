...
def sample_Z(m, n):
    return np.random.uniform(-1., 1., size=[m, n])# uniform 表示从一个均匀分布[low,high)随机抽取一个size大小的ndarry
...
def plot(samples):
    ...
    gs = gridspec.GridSpec(4, 4)#创建一个4X4的区域类似于plot
    gs.update(wspace=0.05, hspace=0.05)
    for i, sample in enumerate(samples):#enumerate函数的功能是把sequeence变成一个enumberate对象，如果seq是"january,feb,jan",那么返回(0,january)...
        ax = plt.subplot(gs[i])
        plt.axis('off')
        ax.set_xticklabels([])
        ax.set_yticklabels([])
