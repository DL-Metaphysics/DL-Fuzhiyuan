    ...
    def set_data(self, train_ratings, test_ratings):
        ...
        Args:
            ...
            test_ratings (:obj:`pd.DataFrame`): `M` testing instances (rows)
                with three columns `[user, item, rating]`
        """
        self.train_inter_pos, self. train_inter_neg = \ # train_inter_pos 和_neg分别是数据中大于或等于平均分的用户组，另一个则是小于的用户组
            get_pos_neg_splits(train_ratings)
        self.pos_level_dist, self.neg_level_dist = \    # pos_level_dist和neg分别是正抽样文本和负抽样文本的概率，通俗的来说是每个评分的比例
...
def get_overall_level_distributions(train_inter_pos, train_inter_neg, beta):
    ...
    pos_level_dist = get_pos_level_dist(pos_counts.index.values,  #.index.values和.values之间的区别是前者是出现的评分，后者是每个评分所出现的次数
                                        pos_counts.values)
